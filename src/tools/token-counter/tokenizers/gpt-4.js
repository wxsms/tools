...
// GPT-4 / GPT-4-turbo / GPT-3.5-turbo / text-embedding-3-* all use the
// cl100k_base BPE vocabulary. The ChatML envelope tokens <|im_start|> /
// <|im_end|> are NOT registered specials in cl100k_base itself (they encode
// to 6 byte-BPE ids each); we register them via getEncoding's
// extendSpecialTokens at the official ids the OpenAI API uses (100264 /
// 100265), so the envelope collapses to single tokens — matching what the
// API actually counts (cookbook GPT-4 rules: +3 per message, +1 if name,
// +3 priming at end).
