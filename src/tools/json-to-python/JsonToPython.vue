<template>
  <div class="json-to-python-page">
    ...
    <div class="flex gap-4">
      ...
      <div class="flex-1 form-control min-w-0">
        <label class="label mb-2">
          <span class="label-text font-semibold">Python 输出</span>
          <button
            v-if="output"
            class="btn btn-ghost btn-xs gap-1"
...
async function generate() {
  ...
  if (r.ok) {
    ...
    // 校验失败时保留上一次成功的 Python 输出,只更新错误信息
    warning.value = ''
    error.value = r.error
  }
}
...
<style>
.json-to-python-page .cm-container {
...
.json-to-python-page .cm-container .cm-editor {
  height: 100%;
  font-size: 0.875rem;
}

.json-to-python-page .cm-container .cm-editor.cm-focused {
  outline: none;
}

:not([data-theme="dark"]) .json-to-python-page .cm-container .cm-editor {
  background: var(--color-base-300);
}
:not([data-theme="dark"]) .json-to-python-page .cm-container .cm-editor .cm-gutters {
  background: var(--color-base-300);
  border-right: 1px solid var(--color-base-100);
}
:not([data-theme="dark"]) .json-to-python-page .cm-container .cm-editor .cm-activeLineGutter {
  background: var(--color-base-200);
}
:not([data-theme="dark"]) .json-to-python-page .cm-container .cm-editor .cm-activeLine {
  background: var(--color-base-200);
}
</style>
